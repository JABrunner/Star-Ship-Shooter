...
def main():
    ...
    def redraw_window():
        WIN.blit(BG, (0,0))
        # draw text
        lives_label = main_font.render(f"Lives: {lives}", 1, (0,200,200))
        level_label = main_font.render(f"Level: {level}", 1, (0,200,200))
        ...
        if lost:
            lost_label = lost_font.render("Mission Failure !!", 1, (255,255,255))
    ...
    while run:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                quit()
                # WASD and Arrow Keys both selected to accomodate for left and right hand players
...
def main_menu():
    ...
    while run:
        WIN.blit(BG, (0,0))
        title_label = title_font.render("Click the mouse to begin...", 1, (0,200,100))
        WIN.blit(title_label, (WIDTH/2 - title_label.get_width()/2, 350))
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
            #Start the game with a left mouse click
